            ...
            <div class="recent-posts">
              <!-- <div class="announcement background-opacity">
                <i class="fa fa-volume-up"
                   aria-hidden="true"></i>
                <div>
                  <div v-for="(notice, index) in $store.state.webInfo.notices"
                       :key="index">
                    {{ notice }}
                  </div>
                </div>
              </div> -->
              <articleList :articleList="articles" id="articlelist"></articleList>
              <Pagination :pagination="pagination" @pageArticles="pageArticles" />
            </div>
          </div>
        </div>
        <!-- 页脚 -->
  ...
  data () {
    return {
      ...
      pagination: {
        current: 1,
        size: 12,
        total: this.$store.getters.articleTotal,
        searchKey: '',
        sortId: null,
      },
      guShi: {
