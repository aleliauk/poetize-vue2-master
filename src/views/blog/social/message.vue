    ...
    <div>
      ...
      <div class="message-in" style="text-align: center">
        ...
        <div>
          ...
          <button v-show="show" style="margin-left: 12px;cursor: pointer;width: 20%; padding: 9px 10px;"
            @click="submitMessage" class="message-input">发射
          </button>
        </div>
      </div>
      <!-- 弹幕 -->
  ...
  methods: {
    ...
    submitMessage () {
      ...
      if (!this.$common.isEmpty(this.$store.state.currentUser) && !this.$common.isEmpty(this.$store.state.currentUser.avatar)) {
        ...
        // 用户登录成功但是没头像
      } else if (!this.$common.isEmpty(this.$store.state.currentUser) && this.$common.isEmpty(this.$store.state.currentUser.avatar)) {
        treeHole.avatar = this.$store.state.webInfo.avatar
        // 游客
      } else {
        treeHole.avatar = 'https://file.aleliauk.top/defaultAvatar/595f67c042c1b_610.jpg'
      }
