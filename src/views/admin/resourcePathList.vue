  ...
  <div>
    <div>
      <div class="handle-box">
        ...
        <el-select clearable v-model="pagination.status" placeholder="状态" class="mrb10">
      ...
      <el-table :data="resourcePaths" border class="table" header-cell-class-name="table-header">
        ...
        <el-table-column label="图标" align="center">
          <template slot-scope="scope">
            <el-image lazy :preview-src-list="[scope.row.icon]" class="table-td-thumb" :src="scope.row.icon"
              fit="cover"></el-image>
          </template>
        </el-table-column>
    ...
    <el-dialog title="图标" :visible.sync="iconDialog" width="25%" :append-to-body="true" destroy-on-close center>
      <div>
        <uploadPicture :isAdmin="true" :prefix="resourcePath.type" @addPicture="addIcon" :maxSize="5" :maxNumber="1">
        </uploadPicture>
      </div>
    </el-dialog>
    ...
    <el-dialog title="图片" :visible.sync="coverDialog" width="25%" :append-to-body="true" destroy-on-close center>
      <div>
        <uploadPicture :isAdmin="true" :prefix="resourcePath.type" @addPicture="addPicture" :maxSize="5" :maxNumber="1">
        </uploadPicture>
      </div>
    </el-dialog>
    ...
    <el-dialog title="资源路径" :visible.sync="addResourcePathDialog" width="50%" :before-close="clearDialog"
      :append-to-body="true" :close-on-click-modal="false" center>
      <div>
        <div>
          <div style="margin-bottom: 5px">资源类型：</div>
          <el-select v-model="resourcePath.type" placeholder="资源路径类型" class="handle-select mrb10"
            @change="getClassify(resourcePath.type)">
            <el-option v-for="(item, i) in resourceTypes" :key="i" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
          <div style="margin-top: 10px;margin-bottom: 5px">分类：</div>
          <el-select v-model="resourcePath.classify" placeholder="分类名称" style="width: 100%;" filterable allow-create
            default-first-option :disabled="!['lovePhoto', 'funny', 'favorites'].includes(resourcePath.type)">
            <el-option v-for="(item, i) in option" :key="i" :label="item.classify" :value="item.classify">
            </el-option>
          </el-select>
          <div style="margin-top: 10px;margin-bottom: 5px">{{ resourcePath.type == 'funny' ? '歌曲' : '标题' }}：</div>
          <el-input :disabled="!['friendUrl', 'lovePhoto', 'funny', 'favorites'].includes(resourcePath.type)"
            maxlength="60" v-model="resourcePath.title"></el-input>
          <div style="margin-top: 10px;margin-bottom: 5px">{{ resourcePath.type == 'funny' ? '歌手' : '简介' }}：</div>
          <el-input :disabled="!['friendUrl', 'funny', 'favorites'].includes(resourcePath.type)" maxlength="1000"
            v-model="resourcePath.introduction"></el-input>
          <div style="margin-top: 10px;margin-bottom: 5px">图标：</div>
          <div style="display: flex">
            <el-input :disabled="!['favorites'].includes(resourcePath.type)" v-model="resourcePath.icon"></el-input>
            <div style="width: 66px;margin: 3.5px 0 0 10px">
              <proButton :info="'上传图标'" @click.native="addResourcePathIcon()" :before="$constant.before_color_1"
          ...
          <div style="display: flex">
            <el-input :disabled="!['friendUrl', 'lovePhoto', 'funny', 'favorites'].includes(resourcePath.type)"
              v-model="resourcePath.cover"></el-input>
            <div style="width: 66px;margin: 3.5px 0 0 10px">
          ...
          <div style="margin-top: 10px;margin-bottom: 5px">{{ resourcePath.type == 'friendUrl' || resourcePath.type ==
            'favorites' ? '网址' : '链接' }}：</div>
          <div style="display: flex">
          ...
          <div style="margin-top: 10px;margin-bottom: 5px">{{ resourcePath.type == 'funny' ? '专辑' : '备注' }}：</div>
          <el-input :disabled="!['funny'].includes(resourcePath.type)" maxlength="1000"
            v-model="resourcePath.remark"></el-input>
        </div>
